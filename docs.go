...
// embeddedDocs holds puddle's user-facing documentation so installed
...
// EnsureInstalled writes the embedded docs into $PUDDLE_HOME/docs and
// returns that directory. Existing files are left untouched when their
// content already matches the embedded copy.
func EnsureInstalled(puddleHome string) (string, error) {
	dir := filepath.Join(puddleHome, "docs")
