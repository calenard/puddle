...
# and moves it into $PUDDLE_PREFIX (defaults to $HOME\bin, added to PATH
...
[CmdletBinding()]
param(
  [string]$Version = $env:PUDDLE_VERSION,
  [string]$Prefix  = $env:PUDDLE_PREFIX
)
...
if ($Version -eq "latest") {
  ...
  $apiHeaders = @{} + $headers
  if (-not $apiHeaders.ContainsKey("User-Agent")) { $apiHeaders["User-Agent"] = "puddle-installer" }
...
$tmp = New-Item -ItemType Directory -Path (Join-Path $env:TEMP ("puddle-install-" + [System.Guid]::NewGuid().ToString("N").Substring(0,8)))
