# scratchpad — example puddle extension (TypeScript, source-run)
...
This copies the manifest + source into `$PUDDLE_HOME/extensions/scratchpad/`.
...
Notes persist as JSONL at `<cwd>/.puddle/scratchpad-notes.jsonl`. The
file is created on first `/note` and survives puddle restarts. Each line
