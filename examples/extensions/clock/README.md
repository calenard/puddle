# clock — example puddle extension (Node, no dependencies)
...
This copies the manifest + script into `$PUDDLE_HOME/extensions/clock/`.
puddle picks it up the next time you launch the TUI.
...
puddle doesn't care which one you use; it just spawns whatever `exec`
