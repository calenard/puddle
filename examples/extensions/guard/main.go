...
//	go build -o guard .
//
// Drop it next to its extension.json under $PUDDLE_HOME/extensions/guard/
// (or `puddle ext install ./guard` from this directory) and the next puddle
...
func main() {
	...
	auditPath := filepath.Join(os.TempDir(), "puddle-guard-audit.log")
