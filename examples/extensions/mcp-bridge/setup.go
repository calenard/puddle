...
func setupHelp() string {
	...
	b.WriteString("Add a known MCP server template to your puddle MCP config.\n\n")
	b.WriteString("Usage:\n")
	...
	b.WriteString("\nDefault target is global: $PUDDLE_HOME/mcp.json. Run /reload-ext after changes.\n")
	return b.String()
}
...
func handleSetup(args []string, cwd string) (string, error) {
	...
	path := filepath.Join(puddleHome(), "mcp.json")
	if target == "project" {
		if cwd == "" {
			return "", fmt.Errorf("--project requires a working directory, but none is known")
		}
		path = filepath.Join(cwd, ".puddle", "mcp.json")
	}
