...
// from two locations:
//
//  1. Global:  $PUDDLE_HOME/mcp.json
//  2. Project: .puddle/mcp.json       (in the current working directory)
//
...
// puddleHome returns the puddle state directory.
func puddleHome() string {
	if h := os.Getenv("PUDDLE_HOME"); h != "" {
		return h
	}
	...
	case "windows":
		return filepath.Join(os.Getenv("APPDATA"), "puddle")
...
func loadConfig(cwd string) (Config, error) {
	...
	// 1. Global config
	globalPath := filepath.Join(puddleHome(), "mcp.json")
	...
	if cwd != "" {
		projectPath := filepath.Join(cwd, ".puddle", "mcp.json")
