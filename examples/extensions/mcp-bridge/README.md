...
Connect puddle to [MCP (Model Context Protocol)](https://modelcontextprotocol.io) servers.

This extension reads MCP server configurations from standard locations (same format as Claude Desktop, Cursor, Cline, etc.) and bridges their tools into puddle so the LLM can call them directly.
...
4. **Restart puddle.** On first run the extension refreshes its tool cache in the background. When puddle shows `MCP tool cache changed`, run `/reload-ext` once. Future launches register the cached MCP tools immediately.
...
| Location | Scope |
|---|---|
| `$PUDDLE_HOME/mcp.json` | Global (`$XDG_STATE_HOME/puddle/mcp.json` when `XDG_STATE_HOME` is set) |
| `.puddle/mcp.json` | Project-level (in your project root) |

### Config Format

Standard MCP config — same as Claude Desktop, with puddle-specific extensions:
...
3. **Reload**: if the cache changed, run `/reload-ext` once so puddle rebuilds the tool registry with the new definitions
...
Tools are namespaced to avoid collisions with puddle's built-in tools:
...
1. **On startup**: cached tool definitions are registered without blocking puddle startup
...
# Run without installing (for one puddle session)
puddle --ext .
