// bridge.go — MCP tool → puddle tool registration and routing.
//
// Converts MCP tools into puddle-registered tools with namespaced names:
//
//	mcp__<server>__<tool>
//
// The double underscore separates the server name from the tool name,
// avoiding collisions with puddle's built-in tools (read, write, edit, bash, skill).
package main
...
// bridge connects MCP servers to puddle's extension protocol.
type bridge struct {
	e   *ext.Extension
	cwd string
	...
	mapping map[string]toolMapping // puddle tool name → MCP server + tool; guarded by mu
	logger  *log.Logger

	mu sync.Mutex // guards mapping
}

// newBridge creates a new MCP→puddle bridge.
...
// sanitizeName converts a string into a valid puddle tool name component.
// Puddle tool names must match [a-zA-Z][a-zA-Z0-9_]*.
...
// toolName builds the namespaced puddle tool name for an MCP tool.
...
func (b *bridge) registerCachedTool(serverName string, tool cachedTool) {
	puddleName := toolName(serverName, tool.Name)

	b.mu.Lock()
	if existing, exists := b.mapping[puddleName]; exists {
		b.mu.Unlock()
		if existing.serverName != serverName || existing.mcpTool != tool.Name {
			b.logger.Printf("tool name collision: %s already maps to %s/%s; skipping %s/%s",
				puddleName, existing.serverName, existing.mcpTool, serverName, tool.Name)
		}
		return
	}
	b.mapping[puddleName] = toolMapping{serverName: serverName, mcpTool: tool.Name}
	b.mu.Unlock()

	b.e.Tool(puddleName, tool.Description, json.RawMessage(tool.Schema), func(args json.RawMessage) ext.ToolResult {
		return b.handleToolCall(puddleName, args)
	})

	b.logger.Printf("registered cached tool: %s → %s/%s", puddleName, serverName, tool.Name)
}
...
// handleToolCall routes a puddle tool call to the appropriate MCP server.
func (b *bridge) handleToolCall(puddleName string, args json.RawMessage) ext.ToolResult {
	b.mu.Lock()
	mapping, ok := b.mapping[puddleName]
	b.mu.Unlock()
	...
	// Convert MCP result to puddle ToolResult
	return mcpResultToPuddle(result)
}
...
// mcpResultToPuddle converts an MCP CallToolResult to a puddle ToolResult.
func mcpResultToPuddle(result *mcp.CallToolResult) ext.ToolResult {
	if result == nil {
...
// startAll starts all configured servers without re-registering their tools with puddle.
...
// the extension process, and puddle reaps the whole process on exit.
