// mcp-bridge — Connect puddle to MCP (Model Context Protocol) servers.
//
...
// into puddle so the LLM can call them.
//
// Config locations:
//   - Global:  $PUDDLE_HOME/mcp.json
//   - Project: .puddle/mcp.json
//
...
func main() {
	...
	// Logger writes to stderr (captured by puddle into ext logs)
...
func startBackgroundToolRefresh(e *ext.Extension, b *bridge, logger *log.Logger, cachePath string, cachedToolCount int) {
	// Refresh discovery in the background so puddle startup is not blocked. Only ask
...
func mcpHelp(b *bridge) string {
	...
	sb.WriteString("\nSetup options\n")
	sb.WriteString("  --global                              Write to $PUDDLE_HOME/mcp.json (default)\n")
	sb.WriteString("  --project                             Write to .puddle/mcp.json\n")
