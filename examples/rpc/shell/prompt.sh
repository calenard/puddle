#!/usr/bin/env bash
# Minimal shell client for the `puddle rpc` JSON protocol.
#
# Usage:
...
# Pipe the command into puddle rpc; pipe its stdout through jq to react
# to events. The trailing `cat` keeps the input pipe open until the
# subprocess exits on its own (after `done`).
{
  if [ -n "${PUDDLECORE_RPC_TOKEN:-}" ]; then
    jq -nc --arg t "$PUDDLECORE_RPC_TOKEN" '{id:"0",type:"hello",token:$t}'
  fi
  echo "$cmd"
  # Block here so stdin stays open until puddle exits.
  cat
} | puddle rpc | while IFS= read -r line; do
