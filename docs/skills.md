...
`SKILL.md` file with a YAML frontmatter header. puddle discovers skills
...
puddle looks in these directories, in priority order, and registers the
...
skill ecosystem works in puddle unchanged. Drop your existing
`.claude/skills/` or `.agents/skills/` directories into a project and
puddle will pick them up.
...
In puddle, run `/skills`. A picker lists every discovered skill with its
...
| ecosystem | path | puddle reads it? |
|---|---|---|
| (native) | `.puddle/skills/<name>/SKILL.md` | yes |
...
already using and puddle tags along.
