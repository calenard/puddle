...
puddle can be extended with custom slash commands by running an external
...
- **Phase 4**: interactive extension-owned panels rendered inside puddle.
...
```python
#!/usr/bin/env python3
# $PUDDLE_HOME/extensions/hello-py/hello.py
import json, sys
...
`theme.json` or `themes/theme.json`, no `exec` is required and puddle does
not spawn a subprocess.

`chmod +x hello.py`, install:

```bash
puddle ext install ./hello-py
```

Restart `puddle`, type `/hellopy`, the agent greets you. Done.

## Built-in extensions

**puddle ships with no extensions installed by default.** A fresh `puddle install` (or `go install`) gives you a clean agent. Extensions are entirely opt-in: you install (or `--ext` for one run) only the ones you want.
...
cd path/to/puddle/examples/extensions/hello && go build -o hello .

# install (copies to $PUDDLE_HOME/extensions/hello/)
puddle ext install path/to/puddle/examples/extensions/hello

# or load straight from the repo for one puddle session
puddle --ext path/to/puddle/examples/extensions/hello
```
...
puddle scans two directories on startup, in this order:

1. **Project-local**: `./.puddle/extensions/<name>/extension.json`
2. **Global**: `$PUDDLE_HOME/extensions/<name>/extension.json`

A project-local extension with the same name wins over a global one.
When `XDG_STATE_HOME` is set on any platform, `$PUDDLE_HOME` defaults to
`$XDG_STATE_HOME/puddle`. Otherwise it defaults to `~/Library/Application Support/puddle/`
on macOS, `~/.local/state/puddle` on Linux, or `%LOCALAPPDATA%\puddle` on Windows.
...
manifest tells puddle how to launch it:
...
| field | meaning |
|---|---|
| `name` | required. how puddle identifies the extension; must match what's sent in the `hello` frame. |
| `version` | optional. shown in `puddle ext list`. |
...
| `description` | optional. shown in `puddle ext list`. |
| `enabled` | optional, defaults to `true`. set to `false` to disable without removing. |

## Lifecycle

1. **Discovery**: puddle reads every `extension.json` in the search dirs.
2. **Spawn**: enabled extensions are launched as subprocesses. stderr
   redirects to `$PUDDLE_HOME/logs/ext-<name>.log` (one file per
   extension, append-mode).
3. **Hello handshake**: the extension's first stdout frame must be
   `hello`; puddle replies with `hello_ack` containing the protocol
...
5. **Runtime**: after `ready`, puddle dispatches `command_invoked` frames
   ...
   focused.
6. **Shutdown**: when puddle exits, it sends `shutdown` and waits up to
   2s for the extension to send `shutdown_ack`. Holdouts are
   SIGTERM'd, then SIGKILL'd.

A crashing extension does not bring down puddle. The slash command it
owned simply stops working until the extension is fixed and puddle is
restarted.
...
Sentinel telling puddle "all initial registrations are flushed". Send it
...
- `"open_panel"` — opens an extension-owned interactive panel inside
  puddle. The panel content lives in `open_panel`.
- `"noop"` — the extension handled it itself (e.g. it pushed
  `notify` frames or kicked off background work). puddle doesn't change
  the UI in response.
...
If `error` is non-empty, puddle renders it as a red status line
regardless of `action`.

#### `submit` (one-way, any time)

Submits text as a user prompt in the interactive host. If the agent is
idle, puddle starts a turn immediately. If a turn is already running, puddle
...
{"type":"hello_ack","protocol_version":1,
 "puddle_version":"0.0.7","provider":"anthropic",
 "model":"claude-opus-4-7","cwd":"/Users/pat/Developer/puddle",
 "extension_dir":"/Users/pat/Developer/puddle/.puddle/extensions/todos",
 "data_dir":"/Users/pat/Developer/puddle/.puddle/extensions/todos"}
```
...
Sent when puddle wants to give the extension a chance to block, modify,
...
Sent when the user closes the focused panel from puddle (for example with
...
Sent during graceful puddle exit (or `/reload-ext` once that lands).
Reply with `shutdown_ack` and then exit.

## Managing extensions from the CLI

```
puddle ext list                    list installed extensions and their state
...
puddle ext logs <name> [-f]        cat / tail the extension's stderr
```

`puddle ext doctor` runs the same discovery path as puddle startup, but reports
...
`puddle ext install <path>` does a recursive copy; `<git-url>` does a
...
and load straight from disk for one puddle session:

```
puddle --ext ./my-extension        # short form: -e ./my-extension
puddle --ext ./a -e ./b            # repeatable
```
...
the extension dies with puddle like any other subprocess.
...
into `$PUDDLE_HOME/extensions/hello/`.

`OnHello` is optional. Use it when configuration or registrations need
host metadata such as `HostInfo.CWD`, `Provider`, `Model`, `PuddleVersion`,
...
registered extension tools become callable without restarting puddle.
...
`puddle ext install <git-url>` clones from any URL you give it. There's
no sandbox in v1; if you need isolation, install only extensions you
trust or run puddle under your platform's sandboxing tool (`bwrap` /
...
- [x] `/reload-ext` slash command (hot-reload without restarting puddle)
