...
puddle ships with built-in providers and a model catalog. You can select models
with `/model`, list them with `puddle --list-models`, and add private models in
`$PUDDLE_HOME/models.json`.

## Login methods

Use `/login` in interactive mode.

- `api key`: stores an API key in `$PUDDLE_HOME/auth.json` when the provider uses a normal key.
...
OAuth tokens are stored in `$PUDDLE_HOME/auth.json` and refreshed when refresh is
available.
...
Bedrock is configured with AWS credentials, not a generic puddle API-key entry.
...
AWS_BEARER_TOKEN_BEDROCK=bedrock-api-key-... AWS_REGION=us-east-1 \
  puddle --provider amazon-bedrock --model anthropic.claude-sonnet-4-5-20250929-v1:0
```

Some Bedrock models require regional inference-profile IDs for on-demand
throughput, such as `us.` or `eu.` prefixed model IDs. puddle rewrites known
...
puddle --provider cloudflare-ai-gateway
```
...
export CLOUDFLARE_API_KEY=...
export CLOUDFLARE_ACCOUNT_ID=...
puddle --provider cloudflare-workers-ai
```
...
puddle --provider azure-openai-responses
```

The provider uses Azure's Responses API. If deployment names differ from puddle
...
Credentials are stored in `$PUDDLE_HOME/auth.json` with user-only permissions
when puddle creates the file.
...
`/login` so puddle writes the correct schema.

## Custom providers and models

Use `$PUDDLE_HOME/models.json` for private models, deployment aliases, local
...
For each request, puddle checks credentials in this order:
...
3. `$PUDDLE_HOME/auth.json`, including custom provider keys saved by `/login`.
