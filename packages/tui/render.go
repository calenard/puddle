...
type Renderer struct {
	...
	// enabled via PUDDLE_INLINE_IMAGES; defaults to false.
	prevHadImage bool
	...
	//
	// The trade-off when keepScrollback is true: stale puddle frames
	// remain in scrollback above the live view, so scrolling up
	// in VS Code's terminal shows old (already-superseded) puddle
...
func (r *Renderer) Draw(lines []string, cursorRow, cursorCol int) {
	...
	// is unreliable. Inline images are opt-in via PUDDLE_INLINE_IMAGES;
...
// DrawLog renders puddle in the terminal's main screen as normal terminal
...
func (r *Renderer) DrawLog(chat, bottom []string, cursorBottomRow, cursorCol int) {
	...
	// In main-screen flow mode puddle normally emits only its logical
	...
	writeFull := func(clear, purgeScrollback bool) {
		if clear {
			...
			if r.keepScrollback {
				...
				// It is safe on the first paint because no puddle output can be
