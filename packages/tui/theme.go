...
// spaces terminals commonly support. Most of puddle's theme still uses
...
// ANSI 256-color palette used by puddle. Defined as numeric codes so we
...
type Theme struct {
	...
	Assistant    int            // label color for the puddle role
	Tool         int
	ToolOut      int
	Error        int
	Warning      int
...
var Dark = Theme{
	...
	Assistant:         117,                  // bright cyan — the puddle label color
	Tool:              114,                  // green
	ToolOut:           245,
	Error:             203,
	Warning:           214,
...
// prefix in the chat ("▌ you", "▌ puddle") and as the editor prompt so
...
// TUI background. Empty means puddle should leave the terminal's
...
// puddle-blue gutter at the very left. The bar lives outside the bubble
...
func (t Theme) UserBubbleRow(content string, width int) string {
	...
	// so it matches the tool-box / app accent and reads as puddle's voice
