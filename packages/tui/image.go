...
// Kitty-graphics support, we use it. The PUDDLE_INLINE_IMAGES env var
// overrides the default:
//
//	PUDDLE_INLINE_IMAGES=off         -> force text fallback
//	PUDDLE_INLINE_IMAGES=placeholder -> force text fallback (alias for off)
//	PUDDLE_INLINE_IMAGES=iterm       -> force iTerm2 protocol
//	PUDDLE_INLINE_IMAGES=kitty       -> force Kitty protocol
//	PUDDLE_INLINE_IMAGES=auto        -> explicit auto-detect (same as default)
func DetectImageProtocol() ImageProtocol {
	switch strings.ToLower(os.Getenv("PUDDLE_INLINE_IMAGES")) {
...
func detectImageProtocolAuto() ImageProtocol {
	...
	// useful. Users can still force a protocol via PUDDLE_INLINE_IMAGES.
	...
	// puddle's full-screen per-row redraw: subsequent row clears erase
	...
	// can still force the protocol with PUDDLE_INLINE_IMAGES=iterm.
...
// terminal cell. PUDDLE_CELL_ASPECT lets users tune inline-image row
...
func CellAspectRatio() float64 {
	v := strings.TrimSpace(os.Getenv("PUDDLE_CELL_ASPECT"))
	if v == "" {
