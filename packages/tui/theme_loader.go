...
// ThemeOption is one selectable theme discovered under $PUDDLE_HOME/themes.
...
// $PUDDLE_HOME/themes/*.json. It carries metadata plus separate overrides
...
// LoadThemeFromHome applies a custom theme from $PUDDLE_HOME/themes/*.json
...
func DetectThemeWithCustom(puddleHome, preferred string, timeout time.Duration) (Theme, string, error) {
	detected := DetectThemeFromBackground(timeout)
	return LoadThemeFromHome(puddleHome, preferred, detected)
}

func LoadThemeFromHome(puddleHome, preferred string, detected Theme) (Theme, string, error) {
	path, err := resolveThemePath(puddleHome, preferred)
	if err != nil || path == "" {
		return detected, "", err
	}
	switch path {
...
func AvailableThemes(puddleHome string) []ThemeOption {
	out := []ThemeOption{
		{Value: "auto", Label: "auto", Description: "detect terminal background and use puddle defaults", Builtin: true},
	...
	paths, _ := themeFilesIn(filepath.Join(puddleHome, "themes"))
	sort.Strings(paths)
...
func ThemeExists(puddleHome, preferred string) bool {
	path, err := resolveThemePath(puddleHome, preferred)
	return err == nil && path != ""
}

func resolveThemePath(puddleHome, preferred string) (string, error) {
	...
	if preferred != "" && !strings.HasPrefix(preferred, "builtin:") {
		...
		if filepath.Ext(preferred) == "" {
			candidates = append(candidates,
				filepath.Join(puddleHome, "themes", preferred+".json"),
			)
		} else if !filepath.IsAbs(preferred) {
			candidates = append(candidates,
				filepath.Join(puddleHome, "themes", preferred),
			)
		}
