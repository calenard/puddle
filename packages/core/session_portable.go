...
// A ".puddlesession" is just a puddle JSONL session file with the meta
// header rewritten so the importing user gets fresh ownership.
const PortableExt = ".puddlesession"

// ExportSession writes the session at srcPath to dstPath as a
// portable .puddlesession file. If dstPath is an existing directory the
// file is created inside it with a name derived from the session's
// meta ("YYYYMMDD-HHMMSS-<first-prompt-excerpt>.puddlesession"). The
...
// ImportSession copies the .puddlesession file at srcPath into the
...
//
// The imported session is a first-class puddle session: it'll show up
...
// filenameFor builds a descriptive .puddlesession filename from the
