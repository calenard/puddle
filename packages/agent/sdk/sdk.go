// Package sdk is the public Go SDK for embedding the puddle agent
...
//
// For a non-Go consumer, run `puddle rpc` and speak the same JSON
...
import (
	...
	"github.com/puddle-dev/puddle/packages/agent"
...
// defaults are read from $PUDDLE_HOME/config.json, env vars, and the
...
// Runtime is one puddle agent session. Safe for use from one goroutine
