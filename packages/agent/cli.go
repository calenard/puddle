...
// Run is the top-level entrypoint for the puddle binary.
func Run(rawArgs []string, version string) error {
	// Subcommand router: `puddle bot ...` is handled separately so the
	...
	if handled, err := runPuddlefileCommand(rawArgs, version); handled {
		return err
	}
...
func runWithArgsRaw(rawArgs []string, version string) error {
	// `puddle rpc` is shorthand for `puddle --rpc` so third-party apps can
...
func setupNonInteractiveExtensions(ctx context.Context, args Args, r *Resolved, version string) (*extensions.Manager, func()) {
	extMgr := extensions.New(PuddleHome(), r.CWD, version, r.Provider, r.Model, nonInteractiveExtHooks{})
...
func runInteractive(ctx context.Context, args Args, version string) error {
	...
	extMgr := extensions.New(PuddleHome(), r.CWD, version, r.Provider, r.Model, extHooks)
	...
	// lives under PuddleHome/swarm so per-agent meta/events survive
	...
	swarmMgr = swarm.New(swarm.Config{
		Root:     filepath.Join(PuddleHome(), "swarm"),
		RepoRoot: r.CWD,
	})
	...
	// defaults the way puddle would have without the overrides.
	...
	changeCWD := func(path string) error {
		...
		// what relaunching `puddle --cwd <path>` would do today.
		if !args.NoSess {
			newRoot := agentSessionsRoot(PuddleHome(), args)
	...
	go func() {
		...
		src := <-CheckForUpdateAsync(PuddleHome(), version)
	...
	theme, _, themeErr := tui.DetectThemeWithCustom(PuddleHome(), initialCfg.Theme, 80*time.Millisecond)
	if themeErr != nil {
		fmt.Fprintln(os.Stderr, "theme load:", themeErr)
		if initialCfg.Theme != "" && !tui.ThemeExists(PuddleHome(), initialCfg.Theme) {
			initialCfg.Theme = ""
	...
	// agents so they don't outlive their parent puddle.
	...
	iv = modes.NewInteractive(modes.InteractiveConfig{
		...
		SkillSnapshot: func() []*skills.Skill {
			...
			list, _ := skills.Discover(PuddleHome(), r.CWD, userHome, args.WithSkills)
	...
	// Signal-driven flush: a SIGTERM / SIGHUP to the puddle process
...
func openOrCreateSession(args Args, r Resolved, ag *core.Agent, version string) (*core.Session, error) {
	...
	// Sweep meta-only files left over from older puddle versions (and from
	...
	sessionsRoot := agentSessionsRoot(PuddleHome(), args)
