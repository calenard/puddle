...
// runUpdateCommand dispatches `puddle update`. Returns (handled=true, err)
...
//  5. Extracts the puddle binary from the archive.
...
func printUpdateHelp() {
	fmt.Fprintln(os.Stderr, `puddle update — replace the current puddle binary with the latest release

usage:
  puddle update           download and install the newest release
  puddle update --check   show what update is available; install nothing
  puddle update --help    show this help

notes:
  * The binary must be writable by the current user. On a system-wide
    install (e.g. /usr/local/bin/puddle owned by root) re-run with sudo.
  ...
  * After the binary is installed, every extension under
    $PUDDLE_HOME/extensions/ that is a git checkout is fast-forward
    ...
    skipped per-extension and never abort the overall update. puddle
...
func runUpdateCheck(version string) error {
	if version == "" || version == "dev" || version == "0.0.0" {
		fmt.Println("puddle: dev build (version 0.0.0) — `puddle update` is disabled")
		return nil
	}
	...
	if !versionLess(current, latest) {
		fmt.Printf("puddle %s is up to date (latest: %s)\n", current, latest)
		return nil
	}
	fmt.Printf("puddle %s -> %s available\n  release: %s\n  run 'puddle update' to install\n", current, latest, url)
	return nil
}
...
func runUpdate(version string) error {
	if version == "" || version == "dev" || version == "0.0.0" {
		return errors.New("dev build (version 0.0.0): `puddle update` is disabled. Build a release tag or download from https://github.com/puddle-dev/puddle/releases")
	}
	current := versionOnly(version)
	...
	fmt.Println("puddle update: querying latest release...")
	...
	if !versionLess(current, latest) {
		fmt.Printf("puddle %s is already up to date.\n", current)
		return nil
	}
	fmt.Printf("puddle update: %s -> %s\n", current, latest)
	fmt.Printf("puddle update: release page %s\n", releaseURL)
	...
	fmt.Printf("puddle update: target asset %s\n", assetName)
	...
	tmp, err := os.MkdirTemp("", "puddle-update-")
	if err != nil {
	...
	fmt.Println("puddle update: downloading checksums.txt...")
	...
	fmt.Println("puddle update: extracting...")
	...
	newBin := filepath.Join(extractDir, "puddle")
	if runtime.GOOS == "windows" {
		newBin = filepath.Join(extractDir, "puddle.exe")
	}
	if st, err := os.Stat(newBin); err != nil || st.IsDir() {
		return fmt.Errorf("extracted archive does not contain a puddle binary at %s", newBin)
	}
	...
	// Resolve symlinks so 'puddle' on $PATH that points at /usr/local/bin
	...
	fmt.Printf("puddle update: replacing %s\n", curBin)
	if err := replaceBinary(curBin, newBin); err != nil {
		return fmt.Errorf("replace binary: %w", err)
	}
	fmt.Printf("puddle update: installed %s\n", latest)
	...
	updateAllExtensions(PuddleHome())
...
func releaseAssetName(version string) (name, format string, err error) {
	...
	case "windows":
		// supported
	default:
		return "", "", fmt.Errorf("unsupported OS for puddle update: %s (download manually from the release page)", goos)
	}
	switch goarch {
	case "amd64", "arm64":
		// supported
	default:
		return "", "", fmt.Errorf("unsupported CPU arch for puddle update: %s", goarch)
	}
	...
	return fmt.Sprintf("puddle_%s_%s_%s.%s", version, goos, goarch, ext), ext, nil
}
...
// pulling in a Go archive lib. Reasoning: puddle already depends on
...
func replaceBinary(cur, newBin string) error {
	...
	if runtime.GOOS == "windows" {
		...
		// it behind. Next `puddle update` cleans it up via the
		// os.Remove(bak) above.
		return nil
	}
