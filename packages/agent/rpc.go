...
//
// Auth: if $PUDDLECORE_RPC_TOKEN is set, the first command must be
...
type rpcServer struct {
	...
	// piping a single 'prompt' command into 'puddle rpc' would race the
...
// compact) has finished, so a quick `echo cmd | puddle rpc` invocation
// still produces full output before the process exits.
func (s *rpcServer) run(in io.Reader) error {
	requireToken := os.Getenv("PUDDLECORE_RPC_TOKEN") != ""
	...
	for sc.Scan() {
		...
		if !s.authed {
			...
			if hello.Token != os.Getenv("PUDDLECORE_RPC_TOKEN") {
