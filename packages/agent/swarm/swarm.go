// Package swarm implements puddle's multi-agent supervisor.
//
// A Swarm manages a set of headless puddle subprocesses ("agents")
...
// this package.
//
// Every agent runs with cwd == the parent puddle's RepoRoot — the
...
// `puddle --swarm-agent ...` so we reuse puddle's own model resolution
...
// to a particular host puddle session id. Pass empty to clear the
...
func (f *Swarm) SpawnReq(ctx context.Context, req SpawnRequest) (*Agent, error) {
	...
	// on linux. Long PUDDLE_HOME paths plus an agent slug blow that cap
	// quickly. Pick the shortest path that still keeps sockets
	// per-swarm-root so two puddle instances on the same machine don't
	...
	// Persist the agent's identity so a later `puddle` invocation can
