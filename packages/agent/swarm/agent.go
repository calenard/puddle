...
type Agent struct {
	...
	// keeps using the same model across puddle restarts.
	Model    string
	Provider string

	// SessionID, when non-empty, scopes the agent to a particular
	// host puddle session: the dashboard only surfaces agents whose
	...
	// puddle process reads from this file to replay the full history.
	EventLogPath string
