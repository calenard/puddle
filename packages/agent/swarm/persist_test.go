...
// identity bits. Without this, Reload on the next puddle launch can't
...
// TestReloadRebuildsDetachedAgents simulates a puddle restart by spawning
...
func TestSwarmAgentArgsIncludesModelFlags(t *testing.T) {
	args := swarmAgentArgs(swarmAgentArgsOpts{
		Exe: "/puddle", Dir: "/wt", SessionPath: "/s.json", InboxPath: "/in.sock",
...
// a puddle restart and the dashboard would leak everything again.
...
// pre-existing agent the moment a user upgraded puddle.
