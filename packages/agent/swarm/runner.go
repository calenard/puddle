...
// execRunner spawns `puddle --swarm-agent <inbox> --session <path>` in
// the host's working directory (Agent.Dir, which is always the parent
// puddle's RepoRoot) and consumes its JSONL event stream on stdout.
//
// Why a long-lived daemon and not `puddle --print`: the supervisor and
...
// for the parent's own agents. /swarm open in a separate puddle would
// read the log directly.
type execRunner struct {
	agent *Agent

	// Command overrides the default `puddle --swarm-agent ...`
	...
	// because the only plausible one (<Dir>/.puddle/session.json)
...
// resolve a default the same way a normal `puddle` invocation does.
...
func (r *execRunner) Run(ctx context.Context, sink Sink) error {
	...
	//      any .puddle/ scratch directory under Dir would litter
	//      their source tree.
	//
	...
	cmd.Env = append(os.Environ(),
		"PUDDLE_SWARM_AGENT_ID="+r.agent.ID,
		"PUDDLE_SWARM_EVENT_LOG="+logPath,
	)
