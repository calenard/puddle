...
// swarm root so two puddle instances on the same machine don't collide.
//
// Strategy:
//
...
//     fits.
//  2. If that's too long, fall back to <tmp>/puddle-swarm-<roothash>/<id>.sock.
...
func inboxSocketPath(root, agentID string) (string, error) {
	...
	tmp := os.TempDir()
	dir := filepath.Join(tmp, "puddle-swarm-"+rootTag(root))
...
// in the tmp-dir name so two parallel puddle instances with different
