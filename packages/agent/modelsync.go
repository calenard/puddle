...
func ModelCachePath() string {
	return filepath.Join(PuddleHome(), "models-cache.json")
}

// UserModelsPath returns the path to the user's models.json override.
func UserModelsPath() string {
	return filepath.Join(PuddleHome(), "models.json")
}
...
// LoadUserModels reads $PUDDLE_HOME/models.json and merges any user-defined
...
func LoadUserModels() {
	...
	for _, w := range warnings {
		fmt.Fprintln(os.Stderr, "puddle:", w)
	}
	provider.SetUserModels(models)
}

// isGatewayProvider returns true for providers whose OpenAI-compatible
// endpoint can accept routed model IDs that are not present in puddle's local
// catalog. Vercel AI Gateway is intentionally not listed here: puddle currently
...
// model IDs because those IDs can be valid even when absent from puddle's catalog.
//
...
func ValidateAndRepairConfig() {
	cfg, err := LoadConfig()
	if err != nil {
		fmt.Fprintf(os.Stderr, "puddle: config.json: %v (using defaults)\n", err)
		return
	}
	changed := false

	if cfg.Provider != "" && !isKnownProvider(cfg.Provider) {
		fmt.Fprintf(os.Stderr, "puddle: config.json: unknown provider %q reset to \"anthropic\"\n", cfg.Provider)
		...
		if _, err := provider.FindModel(cfg.Provider, cfg.Model); err != nil {
			...
			if isGatewayProvider(cfg.Provider) && isGatewayRoutedModelID(cfg.Model) {
				...
				fmt.Fprintf(os.Stderr,
					"puddle: config.json: model %q belongs to provider %q (config has provider=%q); switched model to %q\n",
				...
				fix := defaultModelForProvider(cfg.Provider)
				fmt.Fprintf(os.Stderr,
					"puddle: config.json: model %q not found in the active catalog; switched to %q\n",
					cfg.Model, fix)
				cfg.Model = fix
				changed = true
			}
	...
	if changed {
		if err := SaveConfig(cfg); err != nil {
			fmt.Fprintf(os.Stderr, "puddle: config.json: failed to persist repair: %v\n", err)
		}
	}
}
