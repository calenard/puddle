...
type Args struct {
	...
	// tool isn't registered. Useful for running puddle without any
	// extra context biasing the model.
	NoSkill bool

	// WithSkills controls loading user-installed skills from
	// $PUDDLE_HOME/skills/, .puddle/skills/, .claude/skills/, and
	...
	// AgentName/AgentDataDir/PermissionSet are populated by `puddle run`
	// for local Puddlefile agents. They scope sessions and enforce the
...
// uses the same palette as puddle's TUI; when redirected it falls back to
...
func PrintHelp(version string) {
	...
	var headline string
	if useColor {
		headline = th.AccentBar(th.Assistant) + assistant(tui.Bold("puddle. yet another coding agent harness."))
	} else {
		headline = "puddle. yet another coding agent harness."
	}
	...
	section("modes",
		row{"puddle", "interactive tui"},
		row{"puddle \"prompt\"", "interactive, pre-filled prompt"},
		row{"puddle -p \"prompt\"", "print final text, exit"},
		row{"puddle --json \"prompt\"", "newline-delimited json events, exit"},
		row{"puddle rpc", "json-rpc loop on stdin/stdout (see docs/rpc.md)"},
	)
	section("extensions",
		row{"puddle ext list", "list installed extensions"},
		row{"puddle ext install <path|url>", "install into $PUDDLE_HOME/extensions/"},
		row{"puddle --ext ./path/to/ext", "load an extension for this run only"},
		row{"puddle ext help", "show all extension subcommands"},
	)
	section("self-update",
		row{"puddle update", "download and install the latest release"},
		row{"puddle update --check", "show whether a new release is available"},
	)
	section("telegram",
		row{"puddle telegram-bot setup", "configure a telegram bot (from BotFather)"},
		row{"puddle telegram-bot run", "foreground bridge (ctrl+c to stop)"},
		row{"puddle telegram-bot start", "background bridge (detached)"},
		row{"puddle telegram-bot stop", "stop the background bridge"},
		row{"puddle telegram-bot logs [-f]", "tail the background bridge log"},
		row{"puddle telegram-bot status", "config + running state"},
		row{"puddle telegram-bot reset", "forget saved token"},
		row{"puddle tg ...", "short alias for telegram-bot"},
	)
