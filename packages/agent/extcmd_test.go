...
func TestExtDoctorStaticScanAndRender(t *testing.T) {
	...
	mustWrite(filepath.Join(cwd, ".puddle", "extensions", "disabled", "extension.json"), `{"name":"disabled","exec":"./run.sh","enabled":false}`)
	mustWrite(filepath.Join(cwd, ".puddle", "extensions", "theme", "extension.json"), `{"name":"theme"}`)
	mustWrite(filepath.Join(cwd, ".puddle", "extensions", "theme", "theme.json"), `{"name":"Theme"}`)
	mustWrite(filepath.Join(cwd, ".puddle", "extensions", "bad", "extension.json"), `{bad json`)
	mustWrite(filepath.Join(cwd, ".puddle", "extensions", "dup", "extension.json"), `{"name":"dup","exec":"./project.sh"}`)
