...
func TestReadAgentsContextLoadsGlobalAndAncestors(t *testing.T) {
	root := t.TempDir()
	puddleHome := filepath.Join(root, "puddle-home")
	...
	if err := os.WriteFile(filepath.Join(puddleHome, "AGENTS.md"), []byte("global rule"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	got := readAgentsContext(nested, puddleHome)
...
// catalogue (because they edited models.json or puddle's bundled
...
func TestResolveFallsBackWhenConfiguredModelIsGone(t *testing.T) {
	t.Setenv("PUDDLE_HOME", t.TempDir())
...
// #15: pointing PUDDLE_HOME at a fresh dir drops the persisted
...
func TestResolveEnvOnlyBedrockDiscoveredWithoutConfig(t *testing.T) {
	t.Setenv("PUDDLE_HOME", t.TempDir()) // fresh home: no config.json
