...
// swarm-spawned puddle subprocess. It's intentionally close in shape to
...
//     events.jsonl (see PUDDLE_SWARM_EVENT_LOG) so a separate puddle
...
func runSwarmAgentMode(ctx context.Context, args Args, version string) error {
	...
	// pipe is closed (e.g. parent puddle exited but the agent is still
	...
	if path := os.Getenv("PUDDLE_SWARM_EVENT_LOG"); path != "" {
	...
	if !e.orphan {
		...
		if err == nil {
			...
			if _, werr := e.w.Write(line); werr != nil {
				// Supervisor's stdout pipe is gone (parent puddle exited
