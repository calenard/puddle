...
import "github.com/calenard/puddle/packages/tui"
...
// reads "i'm puddle (vX.Y.Z). ..." so users see which build they're on
// the moment puddle starts. After welcomeVersionDuration the caller
...
func welcomeBanner(th tui.Theme, version string, showVersion bool) []string {
	text := "puddle. yet another coding agent harness."
	if showVersion && version != "" {
		text = "puddle (" + version + "). yet another coding agent harness."
	}
