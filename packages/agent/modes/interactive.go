...
import (
	...
	"github.com/calenard/puddle/packages/agent/modes/telegram"
...
type InteractiveConfig struct {
	...
	// resolved tool_render config / PUDDLE_FLAT_TOOLS env at startup.
	FlatTools bool
	...
	// compact_input config / PUDDLE_COMPACT_INPUT env at startup.
	CompactUser bool
	...
	// Agent is optional. If nil, puddle opens without credentials; the
	...
	// SetKimiCLIFallbackDisabled controls whether puddle may fall back to
	// the official Kimi Code CLI token when puddle has no stored Kimi token.
	...
	// PuddleHome is the root directory for sessions/, used by /sessions
	...
	// ChangeCWD switches the running puddle session's working directory
...
func (i *Interactive) Run(ctx context.Context) error {
	...
	// Keep puddle on the terminal's main screen. We intentionally do not
	...
	if i.agent == nil {
		...
		i.dialog.Open(i.cfg.PuddleHome)
	}
...
func (i *Interactive) buildChatLocked(cols int) []string {
	...
	// the first thing the user sees when opening a new puddle session.
...
func (i *Interactive) redraw() {
	...
	// spinner glyph and its funny-line message share the `puddle`
...
// separate graphics layer; the following blank rows are only puddle's reserved
...
func (i *Interactive) handleKey(ctx context.Context, k tui.Key) (done bool) {
	...
	case tui.KeyCtrlC:
		...
		// a deliberate double-ctrl+c to exit puddle entirely. First
...
// view starts blank, matching what relaunching `puddle --cwd <path>`
...
func (i *Interactive) openSettingsDialog() {
	...
	if themeName != "auto" && !tui.ThemeExists(i.cfg.PuddleHome, themeName) {
		themeName = "auto"
		i.cfg.ThemeName = ""
	...
	themeChoice := 0
	availableThemes := tui.AvailableThemes(i.cfg.PuddleHome)
	...
	items := []settingsItem{
		...
		},
		{
			key:     "theme",
			label:   "color theme",
			desc:    "choose a theme from $PUDDLE_HOME/themes or a loaded extension",
...
func (i *Interactive) applySettingToggle(key string, value bool) {
	...
	case "recursive_file_suggest":
		...
		// without restarting puddle. SetRecursive drops its cache.
...
func (i *Interactive) applyThemeNow(name string) {
	...
	th, applied, err := tui.LoadThemeFromHome(i.cfg.PuddleHome, name, detected)
	if err != nil {
		if i.cfg.SettingsStore != nil {
			_ = i.cfg.SettingsStore.SetTheme("auto")
		}
		i.cfg.ThemeName = ""
		th, applied, _ = tui.LoadThemeFromHome(i.cfg.PuddleHome, "auto", detected)
		i.mu.Lock()
...
func (i *Interactive) runSlash(ctx context.Context, cmd string) (done bool) {
	...
	case "/sessions":
		i.sessionDialog.Open(i.cfg.PuddleHome, i.cfg.CWD)
	case "/jump":
	...
	case "/cd":
		...
		// jumps puddle into that directory without relaunching.
		//
...
func providerSetupInfo(provider string) (string, []string, bool) {
	const docsURL = "https://raw.githubusercontent.com/calenard/puddle/main/docs/providers.md"
	switch provider {
	case "amazon-bedrock":
		return "Amazon Bedrock setup", []string{
			"Amazon Bedrock uses AWS credentials instead of a generic puddle API-key entry.",
...
func (i *Interactive) startTurnWithImages(parent context.Context, prompt string, images []provider.ImageBlock) {
	...
	// scrollOffset, which on terminals that mirror puddle's pane
	...
	go func() {
		...
		// Without this, WriteNewTranscript only fires at puddle exit,
...
func (i *Interactive) openTelegramDialog() {
	...
	if len(items) == 0 {
		i.mu.Lock()
		i.statusErr = "telegram not configured. run `puddle telegram-bot setup` first."
		i.mu.Unlock()
		i.invalidate()
		return
	}
...
func (i *Interactive) telegramMenuItems() []telegramItem {
	cfg, err := telegram.LoadConfig(i.cfg.PuddleHome)
...
func (i *Interactive) telegramConnect() {
	...
	if cfg.BotToken == "" {
		i.mu.Lock()
		i.statusErr = "telegram: no bot token configured. run `puddle telegram-bot setup` first."
		i.mu.Unlock()
		i.invalidate()
		return
	}
	...
	// user can `puddle telegram-bot stop` first, then /telegram connect.
	if pid, alive, _ := telegram.IsRunning(i.cfg.PuddleHome); alive && pid > 0 {
		i.mu.Lock()
		i.statusErr = fmt.Sprintf("telegram: bot daemon already running (pid %d). stop it with `puddle telegram-bot stop` first.", pid)
		i.mu.Unlock()
		i.invalidate()
		return
	}
	i.telegramBridge = &telegram.Bridge{
		...
		Save: func(next telegram.Config) error {
			return telegram.SaveConfig(i.cfg.PuddleHome, next)
		},
...
func (i *Interactive) telegramStatus() {
	...
	} else if pid, alive, _ := telegram.IsRunning(i.cfg.PuddleHome); alive && pid > 0 {
		msg = fmt.Sprintf("telegram: background daemon running (pid %d) - /telegram connect won't work until you stop it", pid)
	} else {
		cfg, _ := telegram.LoadConfig(i.cfg.PuddleHome)
		if cfg.BotToken == "" {
			msg = "telegram: not configured. run `puddle telegram-bot setup` first."
		} else {
...
func (i *Interactive) openSessionOpsDialog() {
	items := []sessionOpsItem{
		{label: "export", action: "export", hint: "write the current session to a .puddlesession file"},
		{label: "import", action: "import", hint: "load a .puddlesession file into this directory"},
...
// doSessionImport copies the .puddlesession file at src into the
...
func (i *Interactive) doSessionImport(src string) {
	...
	if src == "" {
		i.mu.Lock()
		i.statusErr = "import: pass a path — e.g. /session import ~/Downloads/work.puddlesession"
		i.mu.Unlock()
		i.invalidate()
		return
	}
	...
	newPath, err := core.ImportSession(src, i.cfg.PuddleHome, i.cfg.CWD, i.cfg.Version)
	if err != nil {
		i.mu.Lock()
		i.statusErr = "import: " + err.Error()
		i.mu.Unlock()
...
func (i *Interactive) doSessionTree() {
	...
	if current != "" && i.sessionTreeDialog.OpenSessionFamily(i.cfg.PuddleHome, i.cfg.CWD, current) {
		i.invalidate()
		return
	}
	msgs := []provider.Message{}
...
func (i *Interactive) applySessionTreeMessageSelection(src string, msgIdx, turnNo int, role provider.Role, prompt string) {
	...
	newPath, err := core.BranchSessionHidden(src, i.cfg.PuddleHome, i.cfg.CWD, i.cfg.Version, upTo)
	if err != nil {
		i.mu.Lock()
...
func (i *Interactive) applyForkSelection(msgIdx int) {
	...
	upTo := msgIdx + 1
	newPath, err := core.BranchSession(src, i.cfg.PuddleHome, i.cfg.CWD, i.cfg.Version, upTo)
	if err != nil {
		i.mu.Lock()
