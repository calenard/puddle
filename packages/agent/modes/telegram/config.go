// Package telegram implements puddle's Telegram bot bridge.
//
...
//
// State (bot token, allowed user id) lives in $PUDDLE_HOME/bot.json.
package telegram
...
func ConfigPath(puddleHome string) string {
	return filepath.Join(puddleHome, "bot.json")
}

// LoadConfig reads bot.json, returning a zero Config if it doesn't exist.
func LoadConfig(puddleHome string) (Config, error) {
	var c Config
	b, err := os.ReadFile(ConfigPath(puddleHome))
...
func SaveConfig(puddleHome string, c Config) error {
