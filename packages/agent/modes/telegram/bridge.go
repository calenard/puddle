...
type Bridge struct {
	...
	// should be sent bare (no "puddle: " prefix) because the turn was
	// initiated by a Telegram DM. The flag clears as soon as the
	// reply is flushed. TUI-originated turns leave the flag false
	// so the reply is tagged "puddle: " for clarity on the two-sided
	// transcript.
...
type State struct {
	Running  bool
	Username string // bot username, e.g. "puddlebot"
...
func (b *Bridge) Start(parent context.Context) error {
	...
	if b.Config.BotToken == "" {
		b.mu.Unlock()
		return fmt.Errorf("no bot token configured; run `puddle telegram-bot setup` first")
	}
	b.mu.Unlock()
...
// side initiated the turn: TUI-originated turns get "puddle: " so the
// two-sided transcript reads naturally ("you: ..." / "puddle: ..."),
// while Telegram-originated turns send bare text (the user's own
// bubble is already on-screen, a "puddle: " prefix would just add
...
// OnUserTyped mirrors a message the user typed in the puddle TUI into
...
func (b *Bridge) handleUpdate(ctx context.Context, u Update) {
	...
	if paired == 0 {
		if strings.HasPrefix(text, "/start") {
			...
			_ = b.Client.SendMessage(ctx, msg.Chat.ID,
				fmt.Sprintf("paired with @%s. messages you send here now mirror into the puddle tui.", msg.From.Username),
				msg.MessageID)
	...
	case "/start", "/help":
		_ = b.Client.SendMessage(ctx, msg.Chat.ID,
			"mirror is active. send me a message and it'll be forwarded to the puddle tui. commands: /status, /stop, or plain stop.",
			msg.MessageID)
		return
	case "/status":
