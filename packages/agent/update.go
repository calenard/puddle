...
// release ships without spamming the API on every puddle launch.
...
const githubReleasesAPI = "https://api.github.com/repos/calenard/puddle/releases/latest"
...
// updateCache is the on-disk structure written to $PUDDLE_HOME.
...
func CheckForUpdate(ctx context.Context, puddleHome, currentVersion string) UpdateInfo {
	// Dev builds ("0.0.0") never have an update to offer. Skip.
	if currentVersion == "" || currentVersion == "dev" || currentVersion == "0.0.0" {
		return UpdateInfo{}
	}

	cachePath := filepath.Join(puddleHome, updateCheckFile)
...
func CheckForUpdateAsync(puddleHome, currentVersion string) <-chan UpdateInfo {
	ch := make(chan UpdateInfo, 1)
	go func() {
		defer close(ch)
		ctx, cancel := context.WithTimeout(context.Background(), 4*time.Second)
		defer cancel()
		ch <- CheckForUpdate(ctx, puddleHome, currentVersion)
	}()
	return ch
}
...
// Non-numeric components compare as zero, which is fine for puddle's
// x.y.z-only scheme.
