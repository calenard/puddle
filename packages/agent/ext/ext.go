// Package ext is the Go SDK for writing puddle extensions.
//
// An extension is a subprocess that talks to puddle over its stdin/stdout
...
//	package main
//
//	import "github.com/calenard/puddle/packages/agent/ext"
//
//	func main() {
...
// `$PUDDLE_HOME/extensions/hello/`, and puddle picks it up on next launch.
//
// Use OnHello when registration or configuration depends on host metadata
// such as HostInfo.CWD, Provider, Model, PuddleVersion, ExtensionDir, or DataDir.
...
// ToolResult describing what puddle should send back to the model.
...
// Event is a lifecycle notification from puddle. The fields populated
...
// Response tells puddle how to react to a command invocation. Construct
...
// Extension is one puddle extension. Construct with New, register
...
// HostInfo is what the host (puddle) tells us in HelloAck. Useful for
...
// OnHello registers a callback that runs after puddle acknowledges the
// hello frame and before commands/tools are announced. Use it when
// registrations depend on host metadata such as CWD or PuddleHome-derived
...
// Logf writes a line to the extension's stderr, which puddle captures to
// $PUDDLE_HOME/logs/ext-<name>.log. Use this for debug output: anything
...
// ClosePanel tells puddle to close panelID.
...
// Once Run is going, when the user runs /name in puddle, fn is invoked
...
// OpenAI accept). Call this BEFORE Run(); puddle folds extension tools
...
// Notify pushes an info-level status note into puddle's chat without
...
// Run starts the protocol loop. Blocks until stdin closes (puddle has
...
func (e *Extension) Run() error {
	...
	// Send hello first. puddle answers with hello_ack immediately; read it
