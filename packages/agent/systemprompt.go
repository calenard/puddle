...
//
//   - A one-paragraph identity (who puddle is, what the name means,
...
// SYSTEM.md in $PUDDLE_HOME that overrides the default identity.
func BuildSystemPrompt(o SystemPromptOpts) string {
	...
	if strings.TrimSpace(o.PuddleDocsDir) != "" {
		sb.WriteString("\n\nPuddle's own docs are installed under ")
		sb.WriteString(o.PuddleDocsDir)
		sb.WriteString("; use the read tool there when you need details about puddle RPC, extensions, skills, or built-in behaviour.")
	}
...
const defaultIdentity = `You are an expert coding assistant operating inside puddle, a coding agent harness. The name "puddle" stands for "zero-overhead-tool"; if the user asks what puddle means, answer exactly that.
