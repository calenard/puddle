// Package extensions implements the host side of puddle's subprocess
...
//
// Each extension is its own process, communicating with puddle over its
// stdin/stdout in newline-delimited JSON. Stderr is redirected to a
// per-extension log file under $PUDDLE_HOME/logs/. Crashing one
...
// extension's executable. It tells puddle how to launch the extension
...
// extra puddle ext enable command.
func (m Manifest) IsEnabled() bool {
	return m.Enabled == nil || *m.Enabled
}

// Extension is a running extension subprocess and the metadata puddle
// tracks about it.
...
func (m *Manager) searchDirs() []string {
	var dirs []string
	if m.cwd != "" {
		dirs = append(dirs, filepath.Join(m.cwd, ".puddle", "extensions"))
	}
	if m.puddleHome != "" {
		dirs = append(dirs, filepath.Join(m.puddleHome, "extensions"))
	}
	return dirs
}
...
func (m *Manager) loadOne(ctx context.Context, dir string) error {
	...
	if !mf.IsEnabled() {
		// Quietly skip disabled extensions; puddle ext list will show them.
		return nil
	}
...
// `puddle --ext <path>` so extension authors can iterate on a working
// copy without having to `puddle ext install` after every change.
//
...
func (m *Manager) Reload(ctx context.Context, grace time.Duration) ReloadStats {
	...
	// Wait for ready frames. Use the same 3s grace puddle uses at
...
// would gate every other extension's wait too and puddle startup would
	...
	for _, ext := range exts {
		...
		go func(ext *Extension) {
			...
			case <-deadline:
				...
				ext.mu.Unlock()
				fmt.Fprintf(ext.logFile, "[puddle] timed out waiting for ready frame; proceeding\n")
				ext.readyOnce.Do(func() { close(ext.readyCh) })
			}
		}(ext)
	}
...
func (m *Manager) spawn(ctx context.Context, ext *Extension) error {
	logsDir := filepath.Join(m.puddleHome, "logs")
	...
	for {
		...
		if current.Equal(last) {
			...
			ext.readyOnce.Do(func() {
				...
				ext.mu.Unlock()
				fmt.Fprintf(ext.logFile, "[puddle] no ready frame; auto-readying after idle (legacy SDK?)\n")
				close(ext.readyCh)
			})
			return
		}
...
func (m *Manager) readLoop(ext *Extension, scanner *bufio.Scanner) {
	defer func() {
		...
		m.mu.Unlock()
		ext.readyOnce.Do(func() { close(ext.readyCh) })
		fmt.Fprintf(ext.logFile, "[puddle] extension %s read loop exited at %s\n", ext.Manifest.Name, time.Now().Format(time.RFC3339))
	}()
	...
	for scanner.Scan() {
		...
		if err := json.Unmarshal(line, &frame); err != nil {
			...
			fmt.Fprintf(ext.logFile, "[puddle] malformed json from extension: %v\n", err)
			continue
		}
		switch frame.Type {
		...
		case "register_tool":
			...
			if err := json.Unmarshal(line, &rt); err != nil {
				fmt.Fprintf(ext.logFile, "[puddle] bad register_tool frame: %v\n", err)
				continue
			}
			...
			if len(rt.Schema) > 0 {
				...
				if err := json.Unmarshal(rt.Schema, &tmp); err != nil {
					...
					fmt.Fprintf(ext.logFile, "[puddle] tool %q: schema is not valid json (%v); skipped\n", rt.Name, err)
					continue
				}
			}
			m.mu.Lock()
		...
		case "submit_slash":
			...
			if err := json.Unmarshal(line, &s); err == nil {
				...
				if strings.HasPrefix(text, "/") {
					...
					fmt.Fprintf(ext.logFile, "[puddle] submit_slash refused (not a slash command): %q\n", s.Text)
				}
			}
		...
		default:
			...
			fmt.Fprintf(ext.logFile, "[puddle] unknown frame type %q\n", frame.Type)
		}
	}
}
...
// as `puddle ext doctor`. It is read-only and does not change manager behavior.
...
// the rest of puddle for display purposes.
