...
import (
	...
	puddledocs "github.com/puddle-dev/puddle"
...
// defaultModelForProvider returns the model id puddle prefers when the
...
// knownProviders is the set of provider ids puddle recognises. Used by
...
func Resolve(args Args, requireCred bool) (Resolved, error) {
	...
	// in. That way running plain `puddle` after `/login` (any provider) never
	...
	if credErr != nil && !userPickedProvider && provName != "ollama" {
		...
		// pointing PUDDLE_HOME at a fresh home dir. Iteration order of
	...
	if err != nil {
		...
		// from their models.json or upgraded puddle and the id changed.
		...
		// puddle's local catalog yet, for example OpenRouter's
		...
		if isGatewayProvider(provName) && isGatewayRoutedModelID(model) {
			...
			fmt.Fprintf(os.Stderr,
				"puddle: model %q is not in the active catalogue; using %q instead. Pick a different model with --model or /model.\n",
				model, fm.ID)
	...
	if credErr != nil && requireCred {
		return Resolved{}, fmt.Errorf("%w; set %s_API_KEY, pass --api-key, or run `puddle` and /login",
	...
	docsDir, _ := puddledocs.EnsureInstalled(PuddleHome())
	...
	if !args.NoSkill {
		...
		discovered, _ = skills.Discover(PuddleHome(), args.CWD, homeDir, args.WithSkills)
		if len(discovered) > 0 {
	...
	if agentsAddendum := readAgentsContext(args.CWD, PuddleHome()); agentsAddendum != "" {
	...
	//   2. $PUDDLE_HOME/SYSTEM.md (persistent user override)
	//   3. built-in default (defaultIdentity + defaultGuidelines)
	custom := args.SystemPrompt
	if custom == "" {
		custom = readUserSystemPrompt(PuddleHome())
	}
...
// readUserSystemPrompt looks for $PUDDLE_HOME/SYSTEM.md and returns its
...
func readUserSystemPrompt(puddleHome string) string {
	if puddleHome == "" {
		return ""
	}
	path := filepath.Join(puddleHome, "SYSTEM.md")
...
// default file is created or required: puddle only includes files that
// already exist. Global instructions ($PUDDLE_HOME/AGENTS.md) come first,
// followed by project instructions from the top-most parent down to cwd.
func readAgentsContext(cwd, puddleHome string) string {
