...
func TestDiscoverProjectAndGlobalPriorityAndDedup(t *testing.T) {
	tmp := t.TempDir()
	puddleHome := filepath.Join(tmp, "home")
	...
	mk(filepath.Join(cwd, ".puddle", "skills"), "shared", "project version")
	mk(filepath.Join(puddleHome, "skills"), "shared", "global version")
	// Unique skill in global only.
	mk(filepath.Join(puddleHome, "skills"), "global-only", "from global")

	skills, errs := Discover(puddleHome, cwd, "", true /* includeUser */)
	if len(errs) > 0 {
		t.Fatalf("errs: %v", errs)
	}
	// Expect the two user skills + every built-in shipped with the
	// binary (currently the write-puddle-extension authoring guide).
