---
name: write-puddle-themes
description: Help the user create, install, or package puddle themes, including theme-only extensions.
---

# Writing puddle themes

Use this skill when the user asks for help creating, editing,
installing, debugging, or packaging a puddle color theme. Read this skill
before generating a theme file or advising on theme extension layout.

## What a puddle theme is

A puddle theme is a JSON file that overrides any subset of puddle's built-in
...
Valid; inherits all colors/spinner/syntax from puddle defaults.
...
If `colors.light` is missing, puddle applies `colors.dark` overrides on
...
- `assistant` — assistant/puddle accent and spinner text.
...
- `spinner_messages` — list of messages; puddle chooses one per turn.
...
```bash
mkdir -p "$PUDDLE_HOME/themes"
cp my-theme.json "$PUDDLE_HOME/themes/my-theme.json"
```

Then open `/settings` and choose **color theme**. puddle switches theme
immediately and persists the selection in `$PUDDLE_HOME/config.json`.

If a selected theme file is deleted, puddle resets the setting to the
built-in auto/default theme.

## Theme-only extensions

A puddle extension can exist only to ship a theme. No slash command,
