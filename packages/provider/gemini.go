...
// Only the fields puddle actually emits or consumes are declared here.
...
type gemPart struct {
	...
	// from puddle never set this; incoming chunks might.
...
func saveGeminiImageToWorkingDir(mimeType string, data []byte) (string, error) {
	...
	name := "puddle-gemini-image-" + uuid.NewString() + ext
...
// geminiThinkingConfig maps puddle's reasoning level ("low"/"medium"/"high")
