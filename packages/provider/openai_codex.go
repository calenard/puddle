...
//     OpenAI-Beta: responses=experimental, originator: puddle
...
// ---- Responses API wire types (subset needed for puddle's surface) ----
...
func newCodexSessionID() string {
	...
	if _, err := rand.Read(b[:]); err != nil {
		return fmt.Sprintf("puddle-%d", time.Now().UnixNano())
	}
...
func (c *codexClient) Stream(ctx context.Context, req Request) (<-chan Event, error) {
	...
	newReq := func() (*http.Request, error) {
		...
		if codexCLISessionID != "" {
			...
			// metadata. Keep this narrow so Sol retains puddle's proven shape.
			httpReq.Header.Set("originator", "codex_cli_rs")
			httpReq.Header.Set("session-id", codexCLISessionID)
			httpReq.Header.Set("user-agent", "codex_cli_rs/0.0.0")
		} else {
			httpReq.Header.Set("originator", "puddle")
			httpReq.Header.Set("user-agent", fmt.Sprintf("puddle (%s %s)", runtime.GOOS, runtime.GOARCH))
		}
		return httpReq, nil
	}
